...
전체적으로 대형병원의 시스템에선 크게 3가지 비효율성이 존재합니다.
	...
	2) 진료시 환자들이 기본적인 증상을 서술하는데 소모되어 진료에 쓰이지 못하고 낭비되는 시간. 이는 결국 전체적인 진료시간 증가로 이어지며, 대형병원에서 대기시간이 많이 발생하는 주 원인이기도 합니다.
...
'Healthorflow' 서비스를 사용하게 된다면,
	1) 환자는 긴 대기시간을 기다릴 필요 없이 자신의 증상을 자유롭게 서술하면 예약까지 자동으로 이루어지고, 진료시간동안 효율적으로 필요한 질문들을 할 수 있게됩니다.
	2) 의사는 매번 물어보는 공통된 질문을 안해도 되니 좀 더 환자에게 집중할 수 있을 것이며, 이는 진료만족도 증가로 이어집니다.
	3) 병원은 불필요한 인력과 시간을 줄일 수 있으며, 환자에 대한 데이터를 정형화해서 축적할 수 있으므로 이를 빅데이터로써 활용할 수 있을 것입니다.

저희 팀은 주 증상 추출과 관련해 F1-Score기준 92.4%의 정확도를 가지는 프로토타입 모델을 만드는데 성공했으며,
이는 인터넷에 공개된 지식인 및 HiDOC의 데이터만으로 이루어낸 성능이라 실제 병원과 협력해 개발을 진행한다면 이는 더욱 높아질 것으로 예상됩니다.
